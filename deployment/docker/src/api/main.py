...
def _mock_claim_predict(req: ClaimRequest) -> dict:
    ...
    return {"approval_probability": round(prob, 4), "risk_score": round(risk, 4),
            "risk_band": band, "decision": "Approve" if prob >= 0.5 else "Review"}
...
def _mock_fraud_predict(req: FraudRequest) -> dict:
    ...
    return {"is_anomaly": fraud, "anomaly_score": round(score, 4),
            "fraud_flag": fraud, "high_fraud_cluster": fraud,
...
def health_check():
    return {"status": "healthy", "service": "Health Insurance AI Platform",
            "version": "1.0.0", "models_loaded": list(_models.keys())}
...
def analyze_medical_text(req: NLPRequest):
    ...
    try:
        ...
        # Keyword fallback
        diagnosis_map = {
            "hypertension": ("I10", 0.85), "diabetes": ("E11", 0.82),
            "asthma": ("J45", 0.80), "pain": ("M54", 0.70),
            "cancer": ("C34", 0.75), "depression": ("F32", 0.78),
        }
