...
class ClaimFeatureEngineer(BaseTransformer):
    ...
    CATEGORICAL_COLS = ["claim_type", "member_plan", "state", "procedure_code", "diagnosis_code"]
    NUMERICAL_COLS   = [
        "claim_amount", "member_age", "member_bmi", "num_chronic",
        "num_procedures", "days_in_hospital", "prior_auth", "member_smoker"
    ]
    ...
    def transform(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        cat_encoded = np.column_stack([
            self.label_encoders[c].transform(
                df[c].astype(str).fillna("Unknown").map(
                    lambda x: x if x in self.label_encoders[c].classes_ else "Unknown"
                )
            )
            for c in self.CATEGORICAL_COLS
        ])
    ...
    def _base_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["high_cost_flag"]       = (df["claim_amount"] > df["claim_amount"].quantile(0.90)).astype(int)
        ...
        extra = ["claim_per_procedure", "risk_score_age", "risk_score_bmi",
                 "high_cost_flag", "inpatient_flag", "emergency_flag", "composite_risk"]
        for col in extra:
...
class PreprocessingPipeline:
    ...
    def fit_transform_all(self, data: dict) -> dict:
        ...
        missing = [c for c in ["member_age","member_bmi","member_smoker","member_plan","num_chronic"]
                   if c not in claims.columns]
        if missing and not members.empty:
            member_cols = members[["member_id","age","bmi","smoker","plan_type","num_chronic_conditions"]].rename(
                columns={"age":"member_age","bmi":"member_bmi","smoker":"member_smoker",
                         "plan_type":"member_plan","num_chronic_conditions":"num_chronic"})
            claims = claims.merge(member_cols, on="member_id", how="left")

        # Fill any remaining missing columns with safe defaults
        defaults = {"member_age":40,"member_bmi":25.0,"member_smoker":0,
                    "member_plan":"Silver","num_chronic":0}
