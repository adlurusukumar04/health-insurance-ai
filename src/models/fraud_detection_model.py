...
class FraudDetectionModel:
    ...
    def train_isolation_forest(self, X: pd.DataFrame) -> dict:
        ...
        return {"n_anomalies": int(n_anomalies), "anomaly_rate": float(n_anomalies / len(X))}
    ...
    def predict_kmeans(self, X: pd.DataFrame) -> pd.DataFrame:
        ...
        return pd.DataFrame({
            ...
            "high_fraud_cluster": np.isin(clusters, self.fraud_cluster_ids).astype(int),
        }, index=X.index)
    ...
    def association_rule_mining(self, claims_df: pd.DataFrame,
        ...
        try:
            ...
            transactions = (claims_df
                            .groupby("claim_id")[["diagnosis_code", "procedure_code"]]
...
if __name__ == "__main__":
    ...
    print(f"   ✓ Anomaly rate: {iso_result['anomaly_rate']:.1%} | Anomalies found: {iso_result['n_anomalies']:,}\n")
    ...
    for cid, info in km_result["clusters"].items():
        flag = " ← HIGH FRAUD" if info["fraud_rate"] and info["fraud_rate"] > 0.15 else ""
        print(f"      Cluster {cid}: {info['size']:,} claims | fraud rate: {info['fraud_rate']:.1%}{flag}")
