...
class LSTMTrainer:
    def __init__(
        self, num_classes: int = 15, max_vocab: int = 10000, max_len: int = 64
    ):
    ...
    def train(
        ...
        for epoch in range(epochs):
            ...
            logger.info(
                f"  LSTM Epoch {epoch+1}/{epochs} | Loss: {total_loss/len(train_dl):.4f} | Test Acc: {acc:.4f}"
            )
...
if __name__ == "__main__":
    ...
    trainer = LSTMTrainer(
        num_classes=len(notes_df["diagnosis_code"].unique()), max_vocab=5000, max_len=32
    )
